...
chsq_pval <- format(lens_differences_raw_plot$chsq$p.value, digits = 2)
```
...
Chi square tests indicate that the differences in raw material proportions between all three assemblages are highly significant (p = `r chsq_pval`), although the differences in technological composition are not significant (p = `r lens_differences_tech_plot$tech_chi`), most likely due to small sample size.  Individual tests of the differences between the pit lens and the spits above and the pit lens and spits below are also highly significant for raw materials (p = <0.0005), but not for technological categories
