...
directory <- "E:/My Documents/My UW/Research/1206 M2 excavation/Malakunanga 89 excavations/MJB-1989-excavation-paper/"
dates_1989 <- read.csv(paste0(directory, "data/Date_table_from_paper_on_1989_dig.csv"))
```
...
oxcal_medians <- read.csv(paste0(directory, "data/Date_table_from_paper_on_1989_dig_C14_OxCal_output.csv"))
...
p1 <- hist(x2_post, breaks = 100)                   
p2 <- hist(x1_post, breaks = 100) 
...
# put this text in the para
# `r round(summary(slopes_test)[3], 2)`, 95% HDI = `r round(unname(summary(slopes_test)[3, 5:6]),2)`)

```

A test of the difference between the correlation coefficients for the linear regression slopes for 14C and luminescence ages indicates no significant difference between slopes, indicating that both provide effectively identical age-depth relationships for the uppermost two meters of deposit (Bayesian estimation of difference in means = 
<!--- (Insert values from long run) --->
...
Bayesian change point analysis indicates that sedimentation rates slow substantially below 2.0 m to the base of occupation (posterior probability of change is  `r round(hi_probs[1,]$"cp$posterior.prob",2)` at `r round(hi_probs[1,]$age2plot,0)` ka cal BP) and then accelerate again below the lowest occupation (posterior probability of change is `r round(hi_probs[2,]$"cp$posterior.prob",2)` at `r round(hi_probs[2,]$age2plot,0)` ka).
