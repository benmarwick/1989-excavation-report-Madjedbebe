...
# code for lithicsing, if not, uncomment and uplithicse this next line
directory <- "E:/My Documents/My UW/Research/1206 M2 excavation/Malakunanga 89 excavations/MJB-1989-excavation-paper/data/"
lithics <- read.csv(paste0(directory, "Lithics_table_from_paper_on_1989_dig.csv"))
```
...
ggplot(lithics, aes(Spit, Total.Artefacts)) +
  ...
  annotate("text", x = lithics$Spit, y = (lithics$Total.Artefacts + 100), label = round(lithics$C14, 3), size = 4) +
  annotate("text", x = lithics$Spit, y = (lithics$Total.Artefacts + 280), label = round(lithics$OSL, 3), colour = "red", fontface="bold.italic", size = 4) +
  ylim(0,600)

# plot with ka along x-axis to help read peaks

# assumes you run all the code already for working with dates...
cal.date.pr # interpolated ages to known depths



############ problem with plotting ages and total lithics ###

########### problem with two excel files for lithic data that seem to be slightly different! 
...
library(data.table)
dt <- data.table(spit)
spits <- dt[, list(depth = seq(start.depth, end.depth)), by = spit]
...
plot(spit.depth.pr)
spit.depth.pr.df <- data.frame(age = spit.depth.pr, depth = names(spit.depth.pr))
...
# merge interpolated depths to lithics dataframe
interpolated_depths <- merge(x = lithics, 
      y = spit.depth.pr.df, 
      by.x = "Approx.Depth..cm.", 
      by.y = "depth",
      all.x = TRUE)
...
lithology <- with(lithics, cbind(Local.Coarse.Grained.Quartzite, Silcrete, Quartz, Chert))
# get spit numbers
...
# find midppoint of each spit by averaging start and end depth
spit$mid <- (apply(spit[,-1], 1, mean))/100

# interpolate age from midpoint depth of each spit
# using output from the depth-age plot
# interpolate age from loess line with depth

cal.date.pr <- data.frame(age = cal.date.pr, depth = round(spit$mid*100,0))
# determine an age from a known depth
i = 254 # depth in cm, to the nearest 1 cm
cal.date.pr[cal.date.pr$depth == i,] # returns age from loess

# merge depths and ages to lithic data
plot <- merge(plot[c(1:50),], spit, by.x = "Spit", by.y = "spit")
plot <- merge(plot, cal.date.pr, by.x = "mid", by.y = "depth")

# melt for plotting with age on x-axis - put in paper on 1989 data
library(reshape2)
plot.m <- melt(plot, id.vars = "age", measure.vars = c("Quartzite", "Silcrete", "Quartz", "Chert"))
colnames(plot.m) <- c("age", "rawmaterial", "percentage")
plot.m$age <- plot.m$age/1000
library(ggplot2)
...
ggsave(file="F:\\My Documents\\My UW\\Research\\1206 M2 excavation\\Dating\\MKII_lithics_from_1989_for_paper.svg")
